...
**User decisions:** Linux NVIDIA → build llama.cpp from source with CUDA (llama.cpp ships no Linux CUDA prebuilt), falling back to the Vulkan prebuilt → CPU. Linux AMD → ROCm prebuilt first (when a ROCm runtime is detected) → Vulkan → CPU. Scope includes the sh installers, Windows `install.ps1` provisioning parity, and Rust agent startup detection.

**Verified facts:**
- Pinned tag `b10068` (and latest) ships: `ubuntu-rocm-7.2-x64`, `ubuntu-vulkan-{x64,arm64}`, `ubuntu-{x64,arm64}`, `win-cuda-{12.4,13.3}-x64` (+ companion `cudart-llama-bin-win-cuda-*.zip` runtime DLLs), `win-hip-radeon-x64`, `win-vulkan-x64`, `win-cpu-{x64,arm64}`, `macos-{arm64,x64}`. **No Linux CUDA prebuilt** — hence the source build.
...
- `_llx_cuda_build_ready` — `nvcc` + `cmake` + a C++ compiler on PATH.
...
- `llamacpp_detect_backend` — echoes one word:
  - macOS → `metal`
  - Linux: `nvidia-smi -L` succeeds → `cuda` if `_llx_cuda_build_ready` else `vulkan` (log which prereqs are missing); elif `_llx_qualifying_gpu 0x1002` (AMD dGPU or whitelisted APU) → `rocm` if x86_64 + `/dev/kfd` + (`rocminfo`/`rocm-smi`/`/opt/rocm`) else `vulkan`; elif any other qualifying non-AMD, non-NVIDIA drm card (e.g. Intel Arc with ≥ 4 GiB VRAM) → `vulkan`; else `cpu` — an unqualified iGPU-only machine gets the CPU build, and mere `/dev/dri`/`vulkaninfo` presence no longer counts as a GPU signal. NVIDIA checked before AMD (mixed rigs prefer CUDA; the agent's existing `pick_devices` handles device choice at run time).
...
- `llamacpp_asset_for <backend> <tag>` — maps to asset filename (`rocm` = `ubuntu-rocm-7.2-x64`, x86_64 only — comment that "7.2" must be revisited when bumping the pinned tag); `cuda` → empty (source build).
- `llamacpp_stage_prebuilt <backend> <tag> <stagedir>` — download + extract into a staging dir; nonzero on failure.
- `llamacpp_stage_cuda_build <tag> <stagedir>` — re-validate prereqs (forced mode skips detection), download source tarball `https://github.com/$LLAMACPP_REPO/archive/refs/tags/$tag.tar.gz`, print loud "10–30 minutes" notice, then:
  `cmake -S src -B build -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON -DBUILD_SHARED_LIBS=OFF -DLLAMA_CURL=OFF -DLLAMA_BUILD_TESTS=OFF` and `cmake --build build --target llama-server -j$(nproc || getconf _NPROCESSORS_ONLN || echo 4)`; copy `build/bin/.` into stagedir. Static libs sidestep build-tree rpaths; `CMAKE_CUDA_ARCHITECTURES` defaults to `native` (we build on the target). Don't silence cmake output.
...
- **Idempotency rule:** reuse iff llama-server exists AND marker backend == target AND marker tag == tag. Exception: existing `backend=cuda`, target `vulkan`, binary still passes `--version` → reuse (never discard a working source build because nvcc vanished). Everything else — including missing marker (all legacy installs) — reprovisions once.
- **Provision loop:** for each backend in chain → stage into fresh `mktemp -d` (source build for cuda, prebuilt otherwise) → smoke test in staging → on pass, commit: `$SUDO rm -rf "$LLAMA_DIR"`, copy staging in, write marker, set `LLAMA_BACKEND`/`LLAMA_BIN`, break. Old install is only deleted after a replacement passes its smoke test.
...
- `SERVICE.md` (llamacpp section, ~lines 63–90): detection matrix, CUDA source-build prereqs + build time, fallback chains, `--llamacpp-backend` / `LOCALLMOS_LLAMACPP_BACKEND` (forced = hard-fail), reprovision rule, Windows `-Runtime llamacpp`.
...
A later feature will check for llama.cpp updates and offer one-click updating. This plan is deliberately update-friendly: the marker file (`backend=`/`tag=`) makes the update check a tag comparison; staged provisioning makes updates atomic (a failed CUDA rebuild never destroys the working install); and `provision_llamacpp` is re-runnable with a new `LLAMACPP_VERSION`, so the updater reuses this exact code path. Known cost: CUDA rigs pay a 10–30 min on-device rebuild per update and depend on the toolchain staying installed. Designated future fix: publish self-hosted Linux CUDA prebuilts from LocalLMOS release CI and point CUDA rigs at them via the existing `LLAMACPP_REPO` override — CUDA updates then become plain asset downloads. Nothing in this plan blocks that; do not remove the `LLAMACPP_REPO` indirection.
...
- **CUDA path:** on a rig with the toolkit (or after installing `nvidia-cuda-toolkit`): auto mode → source build → marker `backend=cuda`; broken stub `nvcc` → build failure falls back to vulkan in auto mode.
